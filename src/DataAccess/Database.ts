...
export class Database {
    ...
    const indices = [
      {
        ...
        script: `CREATE PRIMARY INDEX \`IX_PRIMARY_DEFAULT\` ON \`${bucketName}\` USING GSI;`,
      },
    ]
  ...
  public ensureAlive(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      this.bucket.query(
        N1qlQuery.fromString(n1ql).adhoc(false).consistency(N1qlQuery.Consistency.REQUEST_PLUS),
      ...
      this.documentMapper.ensureIndices(true, (error: CouchbaseError | null) => {
        if (error) {
          ...
          if (error.code === 5000 && error.message.match(/Index build will retry in background./)) {
            log.error(
              `Non-critical error when attempting to set / update secondary indices: ${error}`
            )
            return resolve()
          }
