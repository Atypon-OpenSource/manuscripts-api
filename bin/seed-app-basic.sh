...
# create two users
  ...
  curl -X "DELETE" "http://localhost:3000/api/v1/container/MPProject:basic-man" \
  --fail \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'manuscripts-app-id: io.manuscripts' \
  ...
  -H 'manuscripts-app-secret: Valid secret' \
  -H "Authorization: Bearer $token" \
  --data-binary "{\"_id\":\"MPProject:basic-man\"}" && \
  echo -e "\"MPProject:basic-man\" created\n"

  curl  -X "POST" "http://localhost:3000/api/v1/container/projects/MPProject:basic-man/manuscripts/MPManuscript:basic-man" \
  --fail \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'manuscripts-app-id: io.manuscripts' \
  -H 'manuscripts-app-secret: Valid secret' \
  -H "Authorization: Bearer $token" && \
  echo -e "\"MPManuscript::basic-man\" created\n"

  parent_path=$(pwd)
  user_specific_file_path="$parent_path/test/data/fixtures/sample/basic.manuscript-json"

  echo $user_specific_file_path

  # cp $file_path $user_specific_file_path
  # if [ "$(uname)" == "Darwin" ]; then
  #     sed -i '' "s/MPProject:B81D5F33-6338-420C-AAEC-CF0CF33E675C/MPProject:basic-man/g" $user_specific_file_path
  ...
  curl "http://localhost:3000/api/v1/project/MPProject:basic-man/save" \
  --fail \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json, */*' \
  -H 'manuscripts-app-id: io.manuscripts' \
 ...
 # rm $user_specific_file_path
  ...
  echo -e "Project\"MPProject:basic-man\" seeded with a manuscript\n"
