...
import { validProject, validProject3 } from '../../../../../data/fixtures/projects'
...
describe('Invitation - invite', () => {
  test('should fail if inviting user does not exist', () => {
    ...
    return expect(invitationService.invite('foo', [], null)).rejects.toThrow(
      InvalidCredentialsError
    )
  })
  ...
  test('should fail if invitedUserEmails is empty', () => {
    ...
    return expect(invitationService.invite('foo', [], null)).rejects.toThrow(ValidationError)
  })
  ...
  test('should fail if inviting user is inviting himself', () => {
    ...
    return expect(invitationService.invite('foo', ['foo@bar.com'], null)).rejects.toThrow(
      ValidationError
    )
  })
  ...
  test('should fail if user profile does not exist', async () => {
    ...
    return expect(invitationService.invite('foo', ['baz@bar.com'], null)).rejects.toThrow(
      ValidationError
    )
  })
  ...
  test('should extend the invitation expiry if invitation already exist', async () => {
    ...
    expect(invitationService.invitationRepository.touch).toHaveBeenCalled()
...
describe('Invitation - inviteToContainer', () => {
  ...
  test('should update the invitation if it is already exist', async () => {
    ...
    expect(containerInvitationService.containerInvitationRepository.patch).toHaveBeenCalled()
