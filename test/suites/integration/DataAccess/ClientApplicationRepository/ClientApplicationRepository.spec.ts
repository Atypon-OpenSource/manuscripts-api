...
import { ClientApplication, clientApplicationsFromSplitString } from '../../../../../src/Models/ClientApplicationModels'
...
describe('Creating application records', () => {
  test('succeeds with valid configuration values', async () => {
    const apps: Array<ClientApplication> = clientApplicationsFromSplitString('x,y,z;y,u,v', ';', ',')
    expect(apps).toBeTruthy()
    await DIContainer.sharedContainer.applicationRepository.ensureApplicationsExist(apps)
    const recoveredApps = await DIContainer.sharedContainer.applicationRepository.getAll({}, { skip: 0, limit: 0, ascOrderBy: [], descOrderBy: [] })
    expect(recoveredApps.length).toBeGreaterThanOrEqual(2)
    const x = await DIContainer.sharedContainer.applicationRepository.getById('x') as ClientApplication
    expect(x._id).toEqual('x')
    expect(x.secret).toEqual('y')
    expect(x.name).toEqual('z')

    const y = await DIContainer.sharedContainer.applicationRepository.getById('y') as ClientApplication
